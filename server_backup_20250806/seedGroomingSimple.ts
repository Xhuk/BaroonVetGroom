...
export async function seedSimpleGroomingData() {
  console.log("Seeding simple grooming records...");
  ...
  try {
    ...
    for (const record of simpleGroomingData) {
      await db.insert(groomingRecords).values(record);
    }
    
    console.log(`✓ Successfully seeded ${simpleGroomingData.length} grooming records`);
  } catch (error) {
